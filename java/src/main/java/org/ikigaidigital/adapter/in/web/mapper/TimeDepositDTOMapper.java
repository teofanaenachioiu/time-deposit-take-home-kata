...
public class TimeDepositDTOMapper {
	...
	public static TimeDepositResponseDTO toDTO(TimeDepositView timeDepositView) {
		return TimeDepositResponseDTO.builder().id(timeDepositView.id()).balance(timeDepositView.balance())
				.planType(timeDepositView.planType()).days(timeDepositView.days())
				.withdrawals(WithdrawalsDTOMapper.toDTO(timeDepositView.withdrawals())).build();
	}
