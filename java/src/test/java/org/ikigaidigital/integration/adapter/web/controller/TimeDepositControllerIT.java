package org.ikigaidigital.integration.adapter.web.controller;
...
class TimeDepositControllerIT {
    ...
    void getAll_shouldReturnAllTimeDepositsWithWithdrawals() throws Exception {
        MvcResult result = mockMvc.perform(get("/api/v1/time-deposits"))
    ...
    private List<TimeDepositResponseDTO> readAndSortResponse(MvcResult result) {
         TimeDepositResponseDTO[] response =
                 objectMapper.readValue(result.getResponse().getContentAsByteArray(), TimeDepositResponseDTO[].class);

        return Arrays.stream(response)
                .sorted(Comparator.comparing(TimeDepositResponseDTO::id))
                .toList();
    }
}
