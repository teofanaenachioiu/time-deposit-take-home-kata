...
@DataJpaTest
@Import(TimeDepositRepositoryAdapter.class)
@AutoConfigureTestDatabase(replace = AutoConfigureTestDatabase.Replace.NONE)
@Testcontainers
@RequiredArgsConstructor(onConstructor_ = @Autowired)
class TimeDepositRepositoryAdapterIT {

	private final TimeDepositRepositoryAdapter adapter;
	...
	void findAllWithWithdrawals_shouldReturnMappedViews() {
		List<TimeDepositView> deposits = adapter.findAllWithWithdrawals().stream()
				.sorted(Comparator.comparing(TimeDepositView::id)).toList();
		...
		assertThat(deposits).extracting(deposit -> deposit.withdrawals().size()).containsExactly(0, 2, 3, 1, 0);
	}

	@Test
	void findAll_shouldReturnMappedDomainDeposits() {
		List<TimeDeposit> deposits = adapter.findAll().stream().sorted(Comparator.comparing(TimeDeposit::getId))
				.toList();
	...
	void save_shouldPersistUpdatedBalances() {
		...
		List<String> persistedPlanTypes = persistedDeposits.stream().map(deposit -> deposit.getPlanType().name())
				.toList();
